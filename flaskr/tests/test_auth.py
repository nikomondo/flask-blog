...
def test_register(client, app):
    ...
    response = client.post("/auth/register", data={"username": "test_a", "password": "test" , "confirm" : "test"})
    ...
    with app.app_context():
        assert db.one_or_404(db.select(User).filter_by(username="test_a")) is not None
